    ...
    def build_encoders_decoders(
        ...
        for ii, name in enumerate(self.fragment_names):
            self.encoders.append(
                MLP(
                    in_dim=self.node_nfs[ii] - self.pos_dim, # 6
                    out_dims=[2 * (self.node_nfs[ii] - self.pos_dim), self.embed_dim], # [12, 6]
                    activation=self.model_config["act_fn"], # swish
                    last_layer_no_activation=True,
                )
            )
            self.decoders.append(
                MLP(
                    in_dim=self.embed_dim, # 6
                    out_dims=[
                        2 * (self.node_nfs[ii] - self.pos_dim), # 12
                        self.node_nfs[ii] - self.pos_dim, # 6
                    ],
