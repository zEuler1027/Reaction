...
class EGNNDynamics(BaseDynamics):
    ...
    def forward(
        ...
        if self.condition_time:
            ...
            h = torch.cat([h, h_time], dim=1) # (num_atoms*3, 7)
            condition_dim += 1

        if self.condition_nf > 0:
            h_condition = conditions[combined_mask]
            h = torch.cat([h, h_condition], dim=1) # (num_atoms*3, 8) one_hot5, charge1, t1, condition1
            condition_dim += self.condition_nf # 2
