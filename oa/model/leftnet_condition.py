...
from oa.model.util_funcs import unsorted_segment_sum
from oa.model.core import MLP
...
class ConditionNet(torch.nn.Module):
    ...
    def __init__(
        ...
        condition_in: Optional[int] = None,
        ...
        **kwargs,
        ...
        # condition
        if self.condition_in is not None:
            emb_channels = in_hidden_channels + condition_emb_dim
            self.condition_emb = nn.Embedding(condition_in, condition_emb_dim)
        else:
            emb_channels = in_hidden_channels

        self.embedding = nn.Linear(emb_channels, hidden_channels)
        self.embedding_out = nn.Linear(hidden_channels, in_hidden_channels)
        self.radial_emb = RBFEmb(num_radial, self.cutoff)
        self.neighbor_emb = NeighborEmb(hidden_channels, emb_channels)
    ...
    def forward(
        ...
        if self.condition_in is not None:
            ...
            h = torch.cat([h, condition_emb], dim=1)

        z_emb = self.embedding(h) # (N, hid)
